...
Usage:
    python conductivity.py msd_Na_600K.dat
...
T         = 600.0       # K
V_ang3    = 3610.8492   # Å³  (equilibrated NPT box volume from thermo output)
...
# ---- Load MSD file ----
fname = sys.argv[1] if len(sys.argv) > 1 else "msd_Na_600K.dat"
...
try:
    ...
    ax.set_ylabel("MSD (Å²)")
    ax.set_title(f"Na MSD at 600 K   σ = {sigma_mS_cm:.2f} mS/cm")
    ax.legend()
    plt.tight_layout()
    plt.savefig("msd_600K.png", dpi=150)
    print("Plot saved to msd_600K.png")
except ImportError:
    pass
